...
- mechanism: 작동원리(각질제거, 진정, 보습, 피지조절, 항산화 등 "어떻게 작용하는가").
...
def _lookup_ingredient_id(conn: psycopg.Connection[Any], name_ko: str) -> int | None:
    with conn.cursor() as cur:
        cur.execute(
            "SELECT ingredient_id FROM ingredients WHERE lower(name_ko) = lower(%s) LIMIT 1;",
            (name_ko,),
        )
        row = cur.fetchone()
...
def resolve_ingredient(conn: psycopg.Connection[Any], surface: str) -> ResolvedNode | None:
    ...
    ingredient_id = _lookup_ingredient_id(conn, surface)
    display = surface
    if ingredient_id is None:
        alias = INGREDIENT_ALIASES.get(surface.lower())
        if alias is not None:
            ingredient_id = _lookup_ingredient_id(conn, alias)
            display = alias
...
def resolve_mechanism(surface: str) -> ResolvedNode | None:
    ...
    if not normalized:
        return None
    return ResolvedNode(
        label="Mechanism", key_prop="name", key_value=normalized, display=normalized
    )
