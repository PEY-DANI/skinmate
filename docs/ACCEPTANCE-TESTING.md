...
### 데이터/문서 (담당 A)
- [ ] **AC-D1** 실데이터 적재 + 유사도 검색 + **제형 게이트**: ≥1 실소스 적재; top-k(k=5) 유사도 결과 비어있지 않고 출처 표기됨; **제형 토큰(에멀전/오일/끈적임/젤…)이 제품 설명의 ≥60%에 존재**; `season_concerns` 시드 비어있지 않음.
...
### 그래프 (담당 A)
- [ ] **AC-G1** Cypher 순회: 시드 관계(계절→고민 포함)가 choke 함수 통해 조회됨.
